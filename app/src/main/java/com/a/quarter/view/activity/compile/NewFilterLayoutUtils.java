...
import com.seu.cameraandimage.common.adapter.FilterAdapter;
import com.seu.cameraandimage.common.bean.FilterInfo;
...
public class NewFilterLayoutUtils {
	...
	public NewFilterLayoutUtils(Context context, MagicDisplay magicDisplay) {
		mContext = context;
	...
	private FilterAdapter.onFilterChangeListener onFilterChangeListener = new FilterAdapter.onFilterChangeListener(){
		...
		public void onFilterChanged(int filterType, int position) {
			...
			NewFilterLayoutUtils.this.position = position;
			...
			if(position <= favouriteFilterInfos.size()){//点击Favourite列表
				for(int i = favouriteFilterInfos.size() + 2; i < filterInfos.size(); i++){
					if(filterInfos.get(i).getFilterType() == Type){
						...
						NewFilterLayoutUtils.this.position = i;
